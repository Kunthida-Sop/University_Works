...
menusorted = sorted(coffeeDB, key=itemgetter(2))
menusorted.reverse()
...
summ_item = []
for a in userorder: #user order = CMAROE
...
if len(summ_item) > 0:
    ...
    print("-"*56)
    total = 0
    for i, val in enumerate(summ_item): #summ_item = [[M,1],[A,2]]
        
        menudetail = [[d[1],d[2]] for d in coffeeDB if val[0] == d[0]] #d0 = CMAROE d1 = name d2=price
        ...
        print("| %2d. | %-30s | %-4s | %6.2f |" % ((i+1),menudetail[0][0],qty,(t_p))) #No.|NAME|QTY|PRICE
    print("-"*56)
    ...
    total2 = total + total_tax

    
    print("| %-43s | %6.2f |" % ("Subtotal",total) )
    print("| %-43s | %6.2f |" % ("Total tax",total_tax) )
    print("| %-43s | %6.2f |" % ("Total",total2) )
    print("-"*56)
else:
    print("NO ORDER!!!")    